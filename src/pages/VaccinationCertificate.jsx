  ...
  return (
    <>
            ...
            {certificates.map((result, index) => (
              <div key={index} className='w-full grid grid-cols-5 gap-3 border border-1 border-gray-200'>
                ...
                <div className='py-8 col-span-2 max-w-auto right-5'>
                  <div className=''>
                    <a href={`/client-details/${result.id}`} className='text-sm font-medium leading-6 text-indigo-600 hover:text-indigo-500'>
                      Download
                    </a>
                  </div>
                </div>
      ...
      <div className='hidden md:block'>
        {!loader && certificates.length > 0 && <Table tableTitle='Vaccination Certificate' theaders={tableHeaders} data={certificates} />}
      </div>
