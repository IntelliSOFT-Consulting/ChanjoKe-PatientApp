...
function Home() {
  ...
  const { loader, appointments, appointmentCount, fetchAppointments } = useAppointment();
  const { certificateCount, fetchUserCertificates } = useCertificate();
  const { recommendations, fetchUserRecommendations } = useImmunizationRecommendation();
  const { immunizationCount, immunizations, fetchPatientImmunizations } = useImmunization();
  ...
  useEffect(() => {
    if (Array.isArray(recommendations) && recommendations.length > 0) {
      ...
      if (userAge < 14) {
        const upcoming = validRecommendations
          .map((vaccine) => {
    ...
    if (recommendations?.length > 0) {
      const unvaccinatedRecommendations = recommendations.filter(
        (recommendation) => !completedImmunizations.includes(recommendation?.vaccineCode?.[0]?.text)
      );
      const passedVaccines = unvaccinatedRecommendations
        .map((vaccine) => {
          const dueDate = vaccine?.dateCriterion?.find((item) => item.code.coding.some((code) => code.code === "Earliest-date-to-administer"));
  ...
  return (
    <div>
      <div className='hidden md:block'>
        <Stats appointments={vaccinationAppointments.length} certificates={certificateCount} vaccines={immunizationCount} />
      </div>
      ...
      <div className='sm:hidden mt-5'>
        {vaccinationAppointments.length > 0 &&
          vaccinationAppointments.map((result, index) => (
      ...
      <div className='hidden md:block'>
        {missedVaccines.length > 0 && !loader && (
          <>
            <h1 className='font-semibold text-1xl mb-3'>Missed Vaccines</h1>
              ...
              locale={{
                emptyText: (
                  <div className='flex flex-col items-center justify-center'>
                    <p className='text-gray-400 text-sm my-2'>User has no missed vaccinations</p>
                  </div>
                ),
              }}
            />
      ...
      <div className='hidden md:block'>
        {vaccinationAppointments.length > 0 && !loader && (
          <>
            <h1 className='font-semibold text-1xl mb-3'>Appointments</h1>
            ...
            <Table
              ...
              locale={{
                emptyText: (
                  <div className='flex flex-col items-center justify-center'>
                    <p className='text-gray-400 text-sm my-2'>No Appointments</p>
                  </div>
                ),
              }}
            />
      ...
      <div className='hidden md:block mt-5'>
        {upcomingVaccinations.length > 0 && !loader && (
          <DefaultTable tableTitle='Upcoming Vaccinations' theaders={tHeaders} data={upcomingVaccinations} />
        )}
      </div>
